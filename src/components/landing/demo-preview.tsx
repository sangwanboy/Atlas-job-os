...
import { LayoutDashboard, BriefcaseBusiness, Bot, Megaphone, FileText } from "lucide-react";

const DEMO_JOBS = [
  { co: "Barclays", role: "Senior Engineer", score: 94, status: "Applied", salary: "\u00a375k\u2013\u00a395k" },
  { co: "BBC", role: "Product Manager", score: 91, status: "Interview", salary: "\u00a365k\u2013\u00a385k" },
  { co: "Revolut", role: "Backend Dev", score: 88, status: "Applied", salary: "\u00a370k\u2013\u00a390k" },
  { co: "Deliveroo", role: "Eng Manager", score: 82, status: "Saved", salary: "\u00a380k\u2013\u00a3110k" },
];

const NAV_ICONS = [LayoutDashboard, BriefcaseBusiness, Bot, Megaphone, FileText];

function statusColor(status: string) {
  switch (status) {
    case "Interview": return "text-emerald-400";
    case "Applied": return "text-cyan-400";
    case "Saved": return "text-amber-400";
    ...
    <section className="py-24 mx-auto max-w-7xl px-6">
      ...
      >
        ...
        <p className="mx-auto mt-4 max-w-xl text-base text-white/45">
          A real-time dashboard that tracks every job, scores every match, and manages your entire pipeline.
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b border-white/[0.08] bg-white/[0.04] px-4 py-3">
          ...
          <div className="ml-4 rounded-md bg-white/[0.06] px-4 py-1 text-xs text-white/30">
            app.atlas-os.co.uk/dashboard
          </div>
        </div>

        {/* Dashboard content */}
        <div className="flex min-h-[400px] bg-[hsl(218_32%_10%)]">
          {/* Mini sidebar */}
          ...
          <div className="flex-1 p-6">
            ...
            <div className="mb-6 grid grid-cols-4 gap-3">
              {[
                ...
                { label: "CVs Generated", value: "3", sub: "DOCX ready" },
                { label: "Outreach Sent", value: "5", sub: "Via Gmail" },
              ].map((kpi) => (
                <div
                  key={kpi.label}
            ...
            <div className="overflow-hidden rounded-xl border border-white/[0.07]">
              <div className="grid grid-cols-5 bg-white/[0.03] px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-white/30">
                <span>Company</span>
                <span>Role</span>
                <span>Salary</span>
                <span>Score</span>
              ...
              {DEMO_JOBS.map((j, i) => (
                <div
                  key={i}
                  className="grid grid-cols-5 border-t border-white/[0.05] px-4 py-2.5 text-xs text-white/60"
                >
                  <span className="font-semibold text-white/80">{j.co}</span>
                  <span>{j.role}</span>
                  <span className="text-white/45">{j.salary}</span>
